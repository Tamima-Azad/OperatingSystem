...
void bestFit(int blk, vector<int> memory, int pr, vector<int> process) {
    ...
    vector<int> remainingBlockSize = memory; 
    vector<bool> blockAllocated(blk, false); 

    for (int i = 0; i < pr; i++) {
        int bestIdx = -1;
        for (int j = 0; j < blk; j++) {
            if (remainingBlockSize[j] >= process[i] and !blockAllocated[j]) {
                if (bestIdx == -1 || remainingBlockSize[j] > remainingBlockSize[bestIdx]) {
                    bestIdx = j;
                }
            }
        }
        if (bestIdx != -1 && !blockAllocated[bestIdx]) {
